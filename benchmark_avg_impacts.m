%Script to evaluate how temporal averaging, temporal extent, and number of daily 
%observations impact the functional relationship from multiple flux tower sites
...
for i = 1:size(files,1)
  ...
  underlineLocation = strfind(files, '_');
  underlineLocation2 = underlineLocation{i};
  underlineLocation3(i) = underlineLocation2(2);
  current_file = files{i};
  firstPart = current_file(8:underlineLocation3(i)-1);
  firstPart_full = current_file(5:underlineLocation3(i)-1);
  site_names(i,1) = cellstr(firstPart);
  site_names_full(i,1) = cellstr(firstPart_full);

  %put data into struct
  this_file = filenames{i};
  sites{i} = readtable(this_file,'TreatAsMissing','NA');
  site_list{i} = struct(site_names{i},sites{i});  
end
site_list = cell2struct(sites,site_names,2);

%Pull all sites in the same time range from 2001-2020. Insert -9999 for missing data
...
for i=1:numel(site_names)
    ...
    X = datevec(site_list.(site_names{i}).TIMESTAMP_START,'yyyymmddHHMM');
    site_list.(site_names{i}).Year = X(:,1);
    site_list.(site_names{i}).Month = X(:,2);
    site_list.(site_names{i}).Day = X(:,3);
    site_list.(site_names{i}).Hour = X(:,4);
    site_list.(site_names{i}).Minute = X(:,5);
    site_list.(site_names{i}).Second = X(:,6);
...
for i=1:numel(site_names)
    ...
    %run [] lines only when calculating q10 at various depths for overlapping time periods
...
%Calculate means over time and construct functional relationships/q10
...
for i=1:numel(site_names)
    ...
    %create half-hourly relationship using 5cm soil temp
    ...
    %create half-hourly relationship using 5cm soil temp during daytime
    ...
    %create daily relationship using 2cm soil temp
    ...
    %create daily relationship using 10cm soil temp
    ...
    %create daily relationship using 5cm soil temp
    ...
    %create weekly relationship using 5cm soil temp
    ...
    %create monthly relationship using 5cm soil temp
    ...
    %create yearly relationship using 5cm soil temp
...
min(q10s_temporal_avg_range)
max(q10s_temporal_avg_range) 
...
%find number of observations used to construct each functional
%relationship when temporally averaging
ns_temporal_avg = [];
for i = 1:16
...
%calculate the percentage of days where measurements are available at each
%site (for table 1)
for i = 1:numel(site_names)
...
temp_diff_daily_avg = mean(temp_diffs_daily);
temp_diff_monthly_avg = mean(temp_diffs_monthly);
temp_range_reduced_avg = mean(temp_ranges_reduced);
temp_range_reduced_yearly_avg = mean(temp_ranges_reduced_yearly);
...
%%%%%%%%%%%%%%%
%How many daily measurements need to be taken each month? Construct
...
%plot randomized functional relationship for days 1, 5, 10, 15, 20, 25 at US-EML
tiledlayout(3,2);
nexttile
for n = 1:500
...
for i = 1:size(site_names,1)
    ...
    if i==16
        ...
        for m = 1:19
            plot(ax3,nan_legend, nan_legend,CT{:,m},'Color',CM(m,:),'LineWidth',3,'DisplayName',leg_names(m))
        end
    else
        b = gca; legend(b,'off');
    end
...
fig2 = figure(); 
tlo2 = tiledlayout(1,1,"TileSpacing","compact");
h2 = gobjects(1,16);
for i = 1:size(sites_yrs_q10_new,2)
    ...
    if i==16
        ...
        "15 years","16 years","17 years","18 years","Full extent"];
        for m = 1:19
            plot(ax3,nan_legend, nan_legend,'-','Color',CM(m,:),'LineWidth',3,'DisplayName',leg_names(m))
        end
    else
        b = gca; legend(b,'off');
    end
...
ix2 = [2,15,1,3,4,6,7,9,14,16,10,11,12,5,8,13]; %index sites based on landcover type from table 1
...
min(cell2mat(q10s_daily))
max(cell2mat(q10s_daily))
...
%Create map of flux tower locations
tower_table = readtable("/Users/jmp838/Desktop/Research/Project 2 - Functional Benchmarks/Flux Towers/flux_tower_lat_lon.csv","TreatAsMissing","NA");
